...
Exist several ways to delimit a calibration area. In [calib_area()](https://sjevelazco.github.io/flexsdm/reference/calib_area.html). We used a
method that the calibration area is delimited by a buffer around presences.
...
data(abies)
abies_p <- abies %>% select(x, y, pr_ab) %>% filter(pr_ab == 1)

somevar[[1]] %>% plot()
points(abies_p %>% select(x, y))
```

This figure shows the species occurrences, using the map of one of the environmental variables (actual evapotranspiration) as background.


```{r calibration area}
ca <- calib_area(data = abies_p, x = 'x', y = 'y', method =  c('buffer', width=80000))
somevar[[1]] %>% plot()
plot(ca, add=TRUE)
points(abies_p %>% select(x, y))
```
...
abies_pf %>% dplyr::group_by(.part) %>% dplyr::count()
...
bg <- lapply(1:4, function(x) sample_background(
    ...
    method = "random",
    rlayer = block_layer, 
    maskval = x, # this argument will constraint sampling process to a given block
    calibarea = ca # this argument will constraint sampling process to the calibration area
  )) %>% 
  bind_rows() 
...
thr_val <- ens_m$performance %>% dplyr::filter(threshold=='max_sens_spec') %>% pull(thr_value)
