...
b <- ggplot() +
  ...
  layer_spatial(crop(as(ca, 'Spatial'), layer1), fill='transparent')+
  geom_point(data = abies_p, aes(x, y), col = "#000004", alpha = 0.5, size = 1) +
  geom_point(data = abies_pf, aes(x, y), col = viridis(5)[4], alpha = 0.8, size = 1) +
  theme_minimal() +
  theme(legend.position = "none", axis.title = element_blank())
b
```
...
b <- ggplot() +
  ...
  scale_fill_gradientn(colours = inferno(8)[c(3, 4, 6, 8)], na.value = NA) +
...
c <- ggplot() +
  ...
  scale_color_gradientn(colours = inferno(8)[c(2, 4, 6, 8)]) +
