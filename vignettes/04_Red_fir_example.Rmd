...
Exist several ways to delimit a calibration area. In [calib_area()](https://sjevelazco.github.io/flexsdm/reference/calib_area.html). We used a method that the calibration area is delimited by a 100-km buffer around presences (shown in the figure below).
...
ca <- calib_area(data = abies_p, x = 'x', y = 'y', method =  c('buffer', width=100000), crs = crs(somevar)) # create a calibration area with 100 km buffer around occurrence points
...
a <- ggplot() +
  layer_spatial(layer1) +
  scale_fill_gradientn(colours = "gray80", na.value = NA) +
  layer_spatial(crop(as(ca, 'Spatial'), layer1), fill='transparent')+
  geom_point(data = abies_p, aes(x, y), col = "#000004", alpha = 0.5, size = 1) +
  theme_minimal() +
...
b <- ggplot() +
  ...
  layer_spatial(crop(as(ca, 'Spatial'), layer1), fill='transparent')+
  geom_point(data = abies_p, aes(x, y), col = "#000004", alpha = 0.5, size = 1) +
  geom_point(data = abies_pf, aes(x, y), col = pals::viridis(5)[4], alpha = 0.8, size = 1) +
  theme_minimal() +
  theme(legend.position = "none", axis.title = element_blank())
b
```
...
block_layer2 <- raster::raster(block_layer)
block_layer2 <- as.factor(block_layer2)
b <- ggplot() +
  layer_spatial(block_layer2) +
  scale_fill_gradientn(colours = pals::inferno(8)[c(3, 4, 6, 8)], na.value = NA) +
  geom_point(data = abies_pf, aes(x, y), col = "#000004", size = 2) +
  theme_minimal() +
  theme(legend.position = "none", axis.title = element_blank(),
...
c <- ggplot() +
  layer_spatial(layer1) +
  scale_fill_gradientn(colours = "gray80", na.value = NA) +
  geom_point(data = bg, aes(x, y, col = (.part)), size = 1, alpha = 0.6) +
  geom_point(data = psa, aes(x, y, col = (.part), shape = "pr_ag"), size = 2) +
  scale_color_gradientn(colours = pals::inferno(8)[c(2, 4, 6, 8)]) +
  geom_point(data = psa, aes(x, y), shape = 1, size = 2) +
  theme_minimal() +
  theme(legend.position = "none", axis.title = element_blank(),
        axis.text = element_blank())
c
...
d <- ggplot() +
  layer_spatial(layer1) +
  scale_fill_gradientn(colours = "gray80",
                       na.value = NA,
                       guide = "none") +
  geom_point(
    data = bg,
  ...
  geom_point(data = abies_pa,
             aes(x, y, col = (as.factor(pr_ab))),
             size = 2) +
  scale_color_discrete(
    type = pals::inferno(8)[c(4, 6)],
    name = "Legend",
...
unconstrained <- raster::raster(pr_1$meanw[[1]])
names(unconstrained) <- "unconstrained"

c1 <- ggplot() +
  layer_spatial(unconstrained) +
  scale_fill_gradientn(colours = rev(pals::viridis(20)), na.value = NA) +
  theme(legend.position = "none", axis.title = element_blank(),
        axis.text = element_blank())
...
constrained <- raster::raster(m_pres$meanw)
names(constrained) <- "constrained"

c2 <- ggplot() +
  layer_spatial(constrained) +
  scale_fill_gradientn(colours = rev(pals::viridis(20)), na.value = NA) +
  theme_minimal() +
  theme(legend.position = "none", axis.title = element_blank(),
        axis.text = element_blank())
