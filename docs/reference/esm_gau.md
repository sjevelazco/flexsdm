...
This function constructs Gaussian Process models using the Ensembles of
Small Models (ESM) approach (Breiner et al., 2015, 2018).

## Usage

``` r
esm_gau(data, response, predictors, partition, thr = NULL, background = NULL)
```
...
- esm_model: A list with "graf" class object for each bivariate model.
  This object can be used for predicting ensembles of small models with
...
combinations of predictors and performs an ensemble based on the average
