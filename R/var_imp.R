...
#'
#' # GGPLOT
#' vip_t %>% pivot_longer(cols = TPR:IMAE,
#' names_to = 'metric',
#' values_to = 'value') %>%
...
var_imp <- function(models,
  ...
  if (length(wm) > 0) {
    ...
    for (i in wm) {
      # prepare real data
      pred_test <- data.frame(pr_ab = data.frame(data)[, response],
      ...
      for (jj in 1:length(predictors)) {
        ...
        var_rand <- foreach::foreach(
          ...
          x <- data.frame(pr_ab = data.frame(data_2)[, response],
                          pred = suppressWarnings(
                            mgcv::predict.gam(
                              m[[i]],
                              newdata = data_2,
  ...
  result[, 4:12] <- result[, 4:12] %>% mutate_if(is.numeric, ~ifelse(. < 0, 0, .))
