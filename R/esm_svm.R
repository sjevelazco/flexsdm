...
#' A list object with:
#' \itemize{
#' \item esm_model: A list with "ksvm" class object from ksvm package for each bivariate model. This object can be used
...
#' \code{\link{esm_glm}}, \code{\link{esm_max}},, and \code{\link{esm_net}}.
#' @export
#'
#' @references
#' \itemize{
...
esm_svm <- function(
  ...
  result <- list(
    ...
    performance = dplyr::left_join(
      tibble(model = "esm_svm", eval_final),
      threshold[1:4],
      by = "threshold"
    ) %>%
