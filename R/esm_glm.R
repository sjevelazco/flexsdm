...
#' A list object with:
#' \itemize{
#' \item esm_model: A list with "glm" class object from stats package for each bivariate model. This object can be used
...
esm_glm <- function(
  ...
  result <- list(
    ...
    performance = dplyr::left_join(
      tibble(model = "esm_glm", eval_final),
      threshold[1:4],
      by = "threshold"
    ) %>%
      dplyr::relocate(model, threshold, thr_value, n_presences, n_absences),
    performance_part = tibble(model = "esm_glm", eval_esm)
  )
