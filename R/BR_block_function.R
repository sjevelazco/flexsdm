## Written by Santiago Velazco and Brooke Rose
...
block_partition_pa <- function(env_layer,
  ...
  # sp_area: shapefile/sf object that delineates species area
  ...
  # Crop and mask raster to spatial extent of species 
  env_crop <- raster::crop(env_layer, sp_area)
  env_mask <- raster::mask(env_crop, sp_area)
  
  # Eliminate those records with NA
  filt <- raster::extract(env_mask, occ_data[,c('x', 'y')])
...
unique list values in pr_ab column are: ",
  ...
  cellSize = seq(res(env_mask[[1]])[1] * min_res_mult, 
                 res(env_mask[[1]])[1] * max_res_mult, 
  ...
  results <-
    foreach(
      ...
      filt <- raster::extract(env_mask, presences2[,c("x", "y")])
      filt <- complete.cases(filt)
      ...
      Env.P <- raster::extract(env_mask, presences2)
      for (i in 1:ncol(part)) {
      ...
      species2 <-
        cbind(presences2@data, raster::extract(env_mask, presences2))
      ...
      for (p in 1:length(grid)) {
        ...
        if (nrow(species2) < 3) {
          ...
          im <- sapply(species2[filt, names(env_mask)],
                       function(x)
                         ape::Moran.I(x,
                                      dist2,
                                      na.rm = TRUE,
